...
help_msg(){
  ...
  echo "        [D]..................Plotter pen's tip dia(in mm)"
  echo ""
  echo ""
  echo "${CYAN}        plotter_gcode -v .. for version"
  echo "        plotter_gcode -h or plotter_gcode --help .. for help"
  echo ""
  echo ""
  echo "${YELLOW}        DEPENDENCIES:"
  echo ""
  echo "        svg2gcode node terminal utility"
  echo "        You can find it here: "
  echo "        https://github.com/em/svg2gcode"
  echo ""
  echo ""
  echo "${RED}        UN-INSTALLATION:"
  echo ""
...
create_gcode(){
  ...
  # rm $fbname".gcode"
  # sleep 4
  echo " ${MAGENTA}:: Converting ${CYAN}$1 ${MAGENTA}to ${CYAN}$fbname.gcode with tip dia: $2mm"
...
add_initial_sequences(){
  ...
  sed -i '' '1s/^/$X\n/g' $fbname".gcode" # [1]
  sed -i '' '1s/^/(Decativate alarm)\n/g' $fbname".gcode"
  sed -i '' '1s/^/ \n/g' $fbname".gcode"
  sed -i '' '1s/^/(Gcode tweaker for plotter. -- Datta)\n/g' $fbname".gcode"
  sleep 4
  sed -i '' 's/n/\
  /g' $fbname".gcode"
...
if [[ "$1" == "-h" || "$1" == "--help" ]];then
  ...
  echo "${CYAN}plotter_gcode version 0.0.1"
elif [[ $# -eq 0 || -z "$1" ]];then
  echo "${RED}No arguments supplied"
  exit
elif [[ $# < 4 || $# > 4 ]];then
  echo "${RED}Some things wrong with argument numbers"
  exit
else
  create_gcode $1 $4
  while :
